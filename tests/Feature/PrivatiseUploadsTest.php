 ...
 *
 * The command reads public/storage as a PHYSICAL folder (not a 'public' disk,
 * which P4 removes), so the test points public_path at a temp dir and writes
 * the legacy files there. The private side is a normal fake.
 */
class PrivatiseUploadsTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->tmp = sys_get_temp_dir().'/he-priv-'.uniqid();
        $this->publicStorage = $this->tmp.'/storage';
        File::ensureDirectoryExists($this->publicStorage);
    ...
    {
        $full = $this->publicStorage.'/'.$relative;
        File::ensureDirectoryExists(dirname($full));
        File::put($full, $body);
    }

    private function publicExists(string $relative): bool
    {
        return File::exists($this->publicStorage.'/'.$relative);
    }
    ...
    {
        $this->putPublic('staff/documents/card.webp', 'AADHAAR');
        ...
        // Copy, not move: the legacy file survives until P4, and the bytes match.
        $this->assertTrue($this->publicExists('staff/documents/card.webp'));
    ...
    {
        $this->putPublic('students/documents/flat.webp', 'AADHAAR');
        $this->putPublic('students/documents/42/deal.webp', 'IMG');
        ...
        StudentDocument::create(['hostel_id' => $this->hostel->id, 'student_id' => $docStudent->id,
            'type' => 'agreement', 'title' => 'A', 'file_path' => 'students/documents/42/deal.webp']);
        ...
        Storage::disk('private')->assertExists("students/{$this->hostel->id}/documents/42/deal.webp");
    }
    ...
    {
        $this->putPublic('registrations/photos/orphan.jpg', 'STRAY');

        $this->assertSame(0, $this->migrate());

        // Reported as an orphan, but never touched — P4 decides its fate.
        $this->assertTrue($this->publicExists('registrations/photos/orphan.jpg'));
    ...
    {
        $this->putPublic('staff/documents/gone.webp', 'AADHAAR');
