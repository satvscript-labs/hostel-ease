...
<div class="row g-3 mb-3">
    <div class="col-md-4"><div class="card stat-card"><div class="card-body py-3"><div class="stat-value">{{ hsms_money($summary['billed']) }}</div><div class="stat-label">Total Billed</div></div></div></div>
    <div class="col-md-4"><div class="card stat-card"><div class="card-body py-3"><div class="stat-value text-success">{{ hsms_money($summary['collected']) }}</div><div class="stat-label">AC Income (Collected)</div></div></div></div>
    <div class="col-md-4"><div class="card stat-card"><div class="card-body py-3"><div class="stat-value text-danger">{{ hsms_money($summary['due']) }}</div><div class="stat-label">AC Due</div></div></div></div>
</div>
        ...
        <table class="table table-hover align-middle mb-0" data-datatable>
            ...
            @foreach($bills as $b)
                <tr>
                    ...
                    <td>{{ hsms_money($b->unit_price) }}</td>
                    <td class="text-end fw-semibold">{{ hsms_money($b->total_amount) }}</td>
                    <td><span class="badge bg-secondary-subtle text-secondary">{{ ucfirst($b->distribution) }} · {{ $b->shares_count }}</span></td>
                    <td class="text-end text-success">{{ hsms_money($b->collected) }}</td>
                    <td class="text-end">
