 ...
 * accounts (Account 360 and every billing action hung off it).
...
return new class extends Migration
{
    public function up(): void
    {
        ...
        DB::table('subscription_accounts')->select('id')->orderBy('id')->chunk(500, function ($rows) {
            foreach ($rows as $row) {
                DB::table('subscription_accounts')->where('id', $row->id)->update(['public_id' => (string) Str::ulid()]);
            }
        });
